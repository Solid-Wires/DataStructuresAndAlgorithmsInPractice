...
def test_insertion_sort_scaling(benchmark, n: int):
    ...
    result: list[int] = benchmark(lambda: run_sort_fn(insertion_sort, pattern["initial"]))
    assert result == pattern["expected"]

# Edge case patterns

def test_insertion_sort_negative_integers():
    pattern: dict = load_pattern(3)
    result: list[int] = run_sort_fn(insertion_sort, pattern["initial"])
    assert result == pattern["expected"]

def test_insertion_sort_contains_zero():
    pattern: dict = load_pattern(4)
    result: list[int] = run_sort_fn(insertion_sort, pattern["initial"])
    assert result == pattern["expected"]

def test_insertion_sort_duplicate_values():
